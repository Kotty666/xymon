...
static char rcsid[] = "$Id: do_alert.c 7360 2014-01-21 16:11:28Z storner $";
...
time_t next_alert(activealerts_t *alert)
{
	...
	while (!stoprulefound && ((recip = next_recipient(alert, &first, NULL, &r_next)) != NULL)) {
		...
		if (recip->criteria && recip->criteria->timespec && !timematch(xmh_item(hinfo, XMH_HOLIDAYS), recip->criteria->timespec)) {
			/* Recipient not active due to time-restrictions. */
			if ((r_next != -1) && (r_next < nexttime)) nexttime = r_next;
		}
		else {
			/* 
